...
encoded as consecutive integers starting from 0. Each line contains (separated by tabulations)
two vertices and a POSITIVE weight that define a weighted edge.
...
Each module m consists of:
    m_p:  a partition module;
    m_ext:  the extension of m_p ((m_p ∩ m_ext)=∅  and (m_p U m_ext) is a module allowing overlaps).

The character ! is necessary to separate these two set of nodes m_p and m_ext:
    before !: the partition module m_p (nodes are separated by tabulations);
    after !: the extension m_ext (nodes are separated by tabulations).
