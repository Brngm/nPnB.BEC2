...
# intrinsic scales
# @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
def equal_C1_C2(C1, C2, eqon):
    """
    complexity when C1=C2: 7*nbv
    """
    if eqon == "equal": # equality is done on real equality C1=C2
            ...
            for mod in C2: # C2 included in C1 # complexity = nbv
                ...
                for i in mod:
                    if not (member1[i] == x):
                        return False
            return True
    else: # equality is not done on real equality C1=C2, but on simple (len(C1)==len(C2)) (it's faster)
        if (len(C1)==len(C2)):
            return True  
        else:
            return False 
        ...
        if (EXIT!=0):
            ...
            if float(X["omega_p"]) == 0:
                if len(INTR) ==0:
                    ...
                    if not equal_C1_C2(X["Cp"], INTR[len(INTR)-1]["Cp"], eqon):
                        INTR.append(X)                    
        s=s+step
    return INTR
