...
const char *BEC(const char *InfileGraph, const char *OutfileClust,
  ...
  out << "omega_p\t" << std::to_string(omega_p) << std::endl; // sum(|adherent(m)|) for m in Cp
  ...
  out << "omega_o\t" << std::to_string(omega_o) << std::endl; // sum(|adherent(m)|) for m in Co
