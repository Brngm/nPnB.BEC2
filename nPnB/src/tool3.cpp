...
void buildOrderedEdges_True_False(graph *g, vect_tuple_ll *OrdEdges) {
  ...
  //
  //     X.sort(key = lambda z : DEG[z["i"]]+(1.0/(z["i"]+1)), reverse = True) # True in buildOrderedEdges_[True]_False for DIC in X:
  //     DIC["n"].sort(key = lambda z : DEG[z]+(1.0/(z+1)), reverse = False) # False in buildOrderedEdges_True_[False]
  //
  //     OrderedEdges=[]
  //     for k in range(nbv):
  //         for DIC in X:
...
long long nodeposition(vect_l *vi, nodeid j) {
  ...
  long long poslow;
  low = std::lower_bound( (*vi).begin(), (*vi).end(), j); // COMPLEXITY: On average, logarithmic in vi.size() 
