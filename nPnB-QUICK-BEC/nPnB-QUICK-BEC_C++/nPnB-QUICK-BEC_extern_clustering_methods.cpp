...
extern "C" {
  const char* overlaps(char* GraphString, char* ClustZero, char* BetaIn, char* RandNode, char* Verbose){
   ...
   with_nodes.resize(clust.cP.size());
   TP_TN_FP_FN = MakeOverlaps(&g, &clust, &clustO, &with_nodes, TP_TN_FP_FN, Beta);
   std::tie (TP, TN, FP, FN) = TP_TN_FP_FN;
   std::tie (Prec, Rec, FSCORE) = PRF(TP, FP, FN, Beta);
   ...
   for(long long i = 0; i < clust.cP.size(); i++) {
     if(clust.aliveP[i]) {
       ...
       long long length_i=with_nodes[i].size();
       long long iii=0;
       for(long long ii = 0; ii < length_i; ii++) {
        iii = iii +1;
        out_stream << std::to_string(with_nodes[i][ii]);
        if(iii != length_i){out_stream << ",";}
      }
       out_stream << "!";
     }
   }
