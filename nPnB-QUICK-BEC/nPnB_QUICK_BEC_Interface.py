...
"""
    ...
    @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
    METRICS:
        PRF(beta, TruePositive, TrueNegative, FalsePositive, FalseNegative)
        return (P, R, F);

        TPTNFPFN_Intrinsic(PathnPnB,  g, Clust)
        return TP, TN, FP, FN

        TPTNFPFN_GOLD_C(PathnPnB, GOLD, Clust, n_vertices)
        return TP, TN, FP, FN;

    CLUSTERING WITHOUT OVERLAPS:
        nPnB_mmm(PathnPnB, g, sp=0.5,  C0string=False, RandNode=False, Epsilon=0.01, Verbose=False)
        return ClustP, Prec, Rec, FSCORE1, FSCOREbeta, TP, TN, FP, FN, Time;

    CLUSTERING ALLOWING OVERLAPS:
        nPnB_overlaps(PathnPnB, g, so=0.15,  C0string=False, RandNode=False, Verbose=False)
        return ClustO, Prec, Rec, FSCORE1, FSCOREbeta, TP, TN, FP, FN, Time;
"""
...
def PRF(beta, TruePositive, TrueNegative, FalsePositive, FalseNegative):
    """
    PRF(beta, TruePositive, TrueNegative, FalsePositive, FalseNegative)
    return (P, R, F)
    """
...
def TPTNFPFN_Intrinsic(PathnPnB,  g, Clust):
    """
    TPTNFPFN_Intrinsic(PathnPnB,  g, Clust)
    return TP, TN, FP, FN
    """
    ...
    stringByte = WRAPP._TPTNFPFN_Intrinsic(PathnPnB, GraphString, ClustString)
...
def TPTNFPFN_GOLD_C(PathnPnB, GOLD, Clust, n_vertices):
    """
    TPTNFPFN_GOLD_C(PathnPnB, GOLD, Clust, n_vertices)
    return TP, TN, FP, FN;
    """
    ...
    stringByte = WRAPP._TPTNFPFN_GOLD_C(PathnPnB, GOLDString, ClustString, n_verticesString)
    string=stringByte.decode("utf-8")
    d1 = '!'
    LINES=string.split(d1)
    TP=float(LINES[0]); TN=float(LINES[1]); FP=float(LINES[2]); FN=float(LINES[3]); 
...
def nPnB_mmm(PathnPnB, g, sp=0.5,  C0string=False, RandNode=False, Epsilon=0.01, Verbose=False):
    """
    nPnB_mmm(PathnPnB, g, sp=0.5,  C0string=False, RandNode=False, Epsilon=0.01, Verbose=False)
    return ClustP, Prec, Rec, FSCORE1, FSCOREbeta, TP, TN, FP, FN, Time
    """
    ...
    #
    if (C0string==False):
        C0string = clust2string([[i] for i in range(g["nbv"])])
    #
    GraphString = graph2string(g)
    CstringP = WRAPP._mmm(PathnPnB, GraphString, C0string, strBetaP, RandNode=RandNode, Epsilon=strEpsilon, Verbose=Verbose)
    ClustP, Prec, Rec, FSCORE1, FSCOREbeta, TP, TN, FP, FN, Time = string2clust(CstringP)
    #
    return ClustP, Prec, Rec, FSCORE1, FSCOREbeta, TP, TN, FP, FN, Time

def nPnB_overlaps(PathnPnB, g, so=0.15,  C0string=False, RandNode=False, Verbose=False):
    """
    nPnB_overlaps(PathnPnB, g, so=0.15,  C0string=False, RandNode=False, Verbose=False)
    return ClustO, Prec, Rec, FSCORE1, FSCOREbeta, TP, TN, FP, FN, Time
    """
    ...
    #
    GraphString = graph2string(g)
    CstringO = WRAPP._overlaps(PathnPnB, GraphString, C0string, strBetaO, RandNode=RandNode, Verbose=Verbose)
    ClustO, Prec, Rec, FSCORE1, FSCOREbeta, TP, TN, FP, FN, Time = string2clust(CstringO)
    #
    return ClustO, Prec, Rec, FSCORE1, FSCOREbeta, TP, TN, FP, FN, Time
...
def graph2string(g):
    ...
    for k in range(len(g["edges"])):
        ((i,j), w) = g["edges"][k]
        if (not g["directed"]):
            assert ((i<g["nbv"]) and (j<g["nbv"])), ("Only %i nodes, but %ith edge: ((%i<-->%i),%.2f)"%(g["nbv"], k,i,j,w))
            assert (not (i==j)), ("No reflexive edge, but %ith edge:  ((%i<-->%i),%.2f)"%(k,i,j,w))
            assert (w > 0), ("Weights must be >0, but %ith edge:  ((%i<-->%i),%.2f)"%(k,i,j,w))
        else:
            assert ((i<g["nbv"]) and (j<g["nbv"])), ("Only %i nodes, but %ith edge: ((%i-->%i),%.2f)"%(g["nbv"], k,i,j,w))
            assert (not (i==j)), ("No reflexive edge, but %ith edge:  ((%i-->%i),%.2f)"%(k,i,j,w))
            assert (w > 0), ("Weights must be >0, but %ith edge:  ((%i-->%i),%.2f)"%(k,i,j,w))
